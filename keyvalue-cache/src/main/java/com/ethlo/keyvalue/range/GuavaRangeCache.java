...
public class GuavaRangeCache<K extends Comparable<K>,V> implements RangeCache<K, V>
{
	...
	public Entry<Range<K>, V> getEntry(K key)
	{
		synchronized (backingMap)
		{
			...
			if (retVal != null)
			{
				...
				return new AbstractMap.SimpleEntry<Range<K>, V>(retVal.getKey(), retVal.getValue().getData());
			}
			return null;
		}
	}
	...
	private void evictIfNecessary()
	{
		...
		if (curCount >= maxSize)
		{
			...
			// Sort
			Collections.sort(accesses, this.evictionPolicy == EvictionPolicy.LRU ? LRU_COMP : LFU_COMP);
	...
	private final Comparator<AccessData<V>> LFU_COMP = new Comparator<AccessData<V>>()
	{
		@Override
		public int compare(AccessData<V> o1, AccessData<V> o2)
		{
			return Long.valueOf(o1.getAccessCount()).compareTo(o2.getAccessCount());
		}
	};
	
	private final Comparator<AccessData<V>> LRU_COMP = new Comparator<AccessData<V>>()
	{
		@Override
		public int compare(AccessData<V> o1, AccessData<V> o2)
		{
			return Long.valueOf(o1.getLastAccessTime()).compareTo(o2.getLastAccessTime());
		}
	};
