...
public abstract class AbstractDataCompressor implements DataCompressor
{
    ...
    public final byte[] compress(final byte[] uncompressed)
    {
        ...
        {
            outputStream.write(UnsignedUtil.unsignedInt(type, 1));
            outputStream.write(UnsignedUtil.unsignedInt(uncompressed.length, 3));
    ...
    {
        final int type = (int) UnsignedUtil.getUnsignedInt(compressed, 0, 1);
        if (type != getType())
        {
        ...
        final int uncompressedLength = Math.toIntExact(UnsignedUtil.getUnsignedInt(compressed, 1, 3));
